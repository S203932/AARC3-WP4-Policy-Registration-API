...
- A MySQL database that has the following structure as seen below.
  - This database will be initialized and populated with dummy data provided one uses the docker image.  
- An IDP capable of doing the OAuth implicit flow and token introspection in accordance with OAuth 2.0. 
...
- When deploying the image on kubernetes or your given platform, provide the aforementioned database, API and IdP information.
...
Only the `GET` operation is supported at this endpoint.
For each policy one will be presented with the `name`, `uri` and `informational_url`.
- name - the name of the policy
- uri - the URI of the given policy 
- informational_url - a link to get further information about the given policy from the API. The link is `/getPolicy/<URI>`.
...
```
If one provides a valid URI, but it is not a known one, then the following would be presented:
```json
{
...
### Main page
From here one is able to call the other endpoints of the API using the ui.
